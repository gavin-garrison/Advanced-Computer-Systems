...
!\[Annotated skew–power Pareto](pareto\_annotated.png)



This figure shows the p99 skew versus average clock power for uniform mesh densities ranging from p\_r = 0 (pure H-tree) to p\_r = 1 (dense local mesh). The pure H-tree exhibits extremely low power but very large skew on the order of hundreds of picoseconds. Introducing sparse local meshes collapses skew by more than three orders of magnitude while increasing power modestly.



Beyond approximately p\_r ≈ 0.25, additional meshing yields diminishing skew improvement while power continues to rise, indicating a clear knee in the design space.
...
!\[Uniform vs adaptive Pareto](pareto\_uniform\_vs\_adaptive.png)
...
!\[Skew vs mesh density](skew\_vs\_pr.png)
...
!\[Skew robustness CDF](robustness\_skew\_cdf.png)



!\[Skew robustness histogram](robustness\_skew\_hist.png)
...
!\[Uniform hybrid arrival heatmap](heat\_uniform\_pr0.25.png)
...
\- pareto\_uniform.csv: uniform mesh density sweep results

\- pareto\_adaptive.csv: region-adaptive sweep results

\- sweep\_results.mat: combined MATLAB output containing all tables and parameters
...
All results and figures can be regenerated by running the following command in MATLAB:
